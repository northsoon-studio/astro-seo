/**
 * buildTags.ts
 * Converts AstroSeoProps into a string of HTML meta/link tags.
...
const buildOpenGraphMediaTags = (
  ...
  media.forEach((medium) => {
    ...
    if (medium.type) {
      tags.push(createOpenGraphTag(`${mediaType}:type`, medium.type));
    }

    if (medium.width) {
      tags.push(createOpenGraphTag(`${mediaType}:width`, medium.width.toString()));
    }

    if (medium.height) {
      tags.push(createOpenGraphTag(`${mediaType}:height`, medium.height.toString()));
    }
  });
  return tags.join("\n");
};
